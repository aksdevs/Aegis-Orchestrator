...
class WorkflowNodes:
    ...
    def __init__(self):
        ...
        # Initialize Vertex AI models for different tasks
        scanner_config = config.get_model_config(ModelType.VULNERABILITY_SCANNER)
        self.vulnerability_scanner = ChatVertexAI(
            project=scanner_config.project_id,
            location=scanner_config.location,
            model_name=scanner_config.model_name,
            temperature=scanner_config.temperature,
            max_output_tokens=scanner_config.max_tokens
        )
        
        researcher_config = config.get_model_config(ModelType.SECURITY_RESEARCHER)
        self.security_researcher = ChatVertexAI(
            project=researcher_config.project_id,
            location=researcher_config.location,
            model_name=researcher_config.model_name,
            temperature=researcher_config.temperature,
            max_output_tokens=researcher_config.max_tokens
        )
        
        fixer_config = config.get_model_config(ModelType.CODE_FIXER)
        self.code_fixer = ChatVertexAI(
            project=fixer_config.project_id,
            location=fixer_config.location,
            model_name=fixer_config.model_name,
            temperature=fixer_config.temperature,
            max_output_tokens=fixer_config.max_tokens
        )
        
        reviewer_config = config.get_model_config(ModelType.CODE_REVIEWER)
        self.code_reviewer = ChatVertexAI(
            project=reviewer_config.project_id,
            location=reviewer_config.location,
            model_name=reviewer_config.model_name,
            temperature=reviewer_config.temperature,
            max_output_tokens=reviewer_config.max_tokens
        )
    ...
    def scan_vulnerabilities(self, state: AegisState) -> AegisState:
        ...
        try:
            ...
            if not self.sast_client:
                self.sast_client = SASTClient(config.project_id, config.location)
